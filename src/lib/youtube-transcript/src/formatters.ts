...
export class SRTFormatter extends TextBasedFormatter {
  ...
  protected _formatTranscriptHelper(index: number, timeText: string, snippet: FetchedTranscriptSnippet): string {
    return `${index + 1}\n${timeText}\n${snippet.text}`;
  }
}
...
export class WebVTTFormatter extends TextBasedFormatter {
  ...
  protected _formatTranscriptHelper(_index: number, timeText: string, snippet: FetchedTranscriptSnippet): string {
    return `${timeText}\n${snippet.text}`;
  }
}
